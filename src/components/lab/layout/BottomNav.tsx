...
export function BottomNav({ active, onChange }: BottomNavProps) {
  return (
    <nav
      aria-label="Primary"
      className="no-print safe-bottom fixed inset-x-0 bottom-0 z-40 border-t border-white/70 bg-white/75 backdrop-blur-2xl"
    >
            ...
            <li key={tab.key} className="flex-1">
              ...
              >
                {isActive && (
                  <motion.span
                    layoutId="nav-pill"
                    className="absolute -top-px h-1 w-10 rounded-full bg-slate-900"
